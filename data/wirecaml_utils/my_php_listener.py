...
from data.wirecaml_utils.phptraverser.php_listener import PHPListener
from data.wirecaml_utils.code_node import CodeNode, Tainted
...
class MyPHPListener(PHPListener):
    ...
    def enter_function_call(self, ctx):
        ...
        func = self.unparse_node(ctx)
        cn = CodeNode(self.get_location(ctx.lineno), func, self.unparse_node(ctx))
        cn.set_vars(self.get_tracked_vars())
        cn.set_funcs(self.get_tracked_funcs())
        cn.set_consts(self.get_tracked_consts())
    ...
    def enter_assignment(self, ctx):
        ...
        if assign in self.get_tracked_vars() and expr.count(assign) == 1:
            self.get_tracked_vars().remove(assign)
    ...
    def exit_if(self, ctx):
        ...
        exit_nodes = [x for x in self.G.nodes if self.G.out_degree(x) == 0 and
    ...
    def get_graph(self):
        ...
        for cn in [x for x in self.G.nodes if x.get_node_type() == FunctionCall]:
